...
HartreePotential::HartreePotential(std::shared_ptr<mrcpp::PoissonOperator> P,
    ...
    println(0, "nucs size " << nucs.size()) println(0, "this->nuclei size " << this->nuclei.size()) this->nuclei = nucs;
    this->local = false;
}
...
void HartreePotential::setupGlobalDensity(double prec) {
    ...
    auto rho_nuc = chemistry::compute_nuclear_density(this->apply_prec, this->nuclei, 1.0e4);
    println(0, "Rho_el int " << rho_el.integrate());
    println(0, "Rho_nuc int " << rho_nuc.integrate());
